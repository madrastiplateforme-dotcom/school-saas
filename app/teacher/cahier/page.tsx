...
export default function TeacherCahierPage() {
  ...
  const loadEntries = async () => {
    ...
    let q = supabase
      .from('cahier_entries')
      .select('id, entry_date, title, content, homework, class_id, subject_id, teacher_id, subjects(id, name), staff(full_name)')
    ...
    const mapped: Entry[] = (data || []).map((e: any) => ({
      ...
      teacher_name: e.staff?.full_name || '—',
      class_id: e.class_id,
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            سجّل الدروس اليومية والعمل المنزلي
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={loadEntries} className="inline-flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2.5 rounded-lg hover:bg-gray-50 font-medium text-sm">
            ...
            <div className="space-y-3">
              {entries.map((e) => (
                <div key={e.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition">
                  <div className={`px-5 py-3 border-b flex items-center justify-between flex-wrap gap-2 ${e.is_mine ? 'bg-sky-50/60 border-sky-100' : 'bg-slate-50 border-slate-100'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {e.is_mine && (
                        <>
                          <button onClick={() => openEdit(e)} className="p-1.5 rounded-md text-slate-500 hover:bg-sky-100 hover:text-sky-700">
                            <Edit3 className="h-3.5 w-3.5" />
                          </button>
                          <button onClick={() => handleDelete(e.id)} className="p-1.5 rounded-md text-slate-500 hover:bg-rose-100 hover:text-rose-700">
