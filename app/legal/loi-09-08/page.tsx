  ...
  return (
    <article className="prose prose-slate max-w-none">
      <header className="mb-10 pb-8 border-b border-slate-200">
        <div className="inline-flex items-center gap-2 rounded-full bg-purple-50 text-purple-700 px-3 py-1 text-xs font-bold mb-4">
        ...
        <p className="mt-3 text-slate-500 text-lg">{t.loi0908.subtitle}</p>
      </header>

      <div className="bg-purple-50 border border-purple-200 rounded-2xl p-6 mb-8">
        <p className="text-purple-900 leading-8">{t.loi0908.intro}</p>
      </div>

      <div className="space-y-8">
        {t.loi0908.sections.map((s, i) => (
          <section key={i}>
            <h2 className="text-xl font-bold text-slate-900 mb-3">{s.title}</h2>
            {'body' in s && s.body && (
              <p className="text-slate-600 leading-8 whitespace-pre-line mb-3">{s.body}</p>
            )}
