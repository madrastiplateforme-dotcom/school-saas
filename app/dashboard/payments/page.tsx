...
export default function NewPaymentPage() {
  ...
  const fetchInitialData = async (sid: string) => {
    ...
    // 🔒 Caisses (filtered 3la 7sab rôle)
    ...
    if (isSecretary) {
      // Secrétaire : ghir caisse dyalha
      caisseQuery = caisseQuery.eq('owner_user_id', user.id)
    } else if (isDirector) {
      // Directeur : ghir Caisse Principale
  ...
  const handleSavePayment = async () => {
    ...
    // 🔒 Koul wa7ed (Secrétaire wla Directeur) : caisse dyalou auto
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {!canSelectCaisse ? (
              // 🔒 Secrétaire wla Directeur : info box (caisse auto)
              <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-3 h-11">
              ...
              // Super Admin f assistance : dropdown
              <select
