...
import {
  ...
  AlertCircle, BookOpen, FileText, MessageSquare, Wallet,
  GraduationCap, Home as HomeIcon, Shield, Users,
} from 'lucide-react'
...
const NOTIF_TYPES = [
  { key: 'absence',    labelAr: 'الغياب',                 labelFr: 'Absence',          icon: AlertCircle },
  { key: 'grade',      labelAr: 'نقطة جديدة',             labelFr: 'Nouvelle note',    icon: GraduationCap },
  { key: 'payment',    labelAr: 'قسط / أداء',             labelFr: 'Paiement',         icon: Wallet },
  { key: 'message',    labelAr: 'رسالة جديدة',            labelFr: 'Message',          icon: MessageSquare },
  { key: 'homework',   labelAr: 'فرض منزلي',              labelFr: 'Devoir',           icon: FileText },
  { key: 'cahier',     labelAr: 'درس جديد',               labelFr: 'Cahier de textes', icon: BookOpen },
  { key: 'discipline', labelAr: 'مخالفة / سلوك',          labelFr: 'Discipline',       icon: Shield },
  { key: 'meeting',    labelAr: 'لقاء مع الأستاذ',        labelFr: 'Réunion',          icon: Users },
]
...
const DEFAULT_PREFS: Prefs = {
  email: { absence: true, grade: true, payment: true, message: true, homework: true, cahier: false, discipline: true, meeting: true },
  in_app: { absence: true, grade: true, payment: true, message: true, homework: true, cahier: true, discipline: true, meeting: true },
  push:  { absence: true, grade: true, payment: true, message: true, homework: true, cahier: false, discipline: true, meeting: true },
}
...
export default function ParentSettingsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (stored) {
        // merge with defaults (ex: nouveau type ajouté après)
        setPrefs({
      ...
      console.error('[parent-settings]', e)
    ...
    <div className="p-6 space-y-6" dir="rtl">
      <header className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            تحكم في نوع ووسيلة الإشعارات اللي كتوصلك
          </p>
        </div>
        <button
          onClick={loadData}
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-indigo-900">
          ...
          <p className="text-indigo-800">
            الإشعارات اللي كتختارها غادي توصلك حسب الوسيلة المختارة. تقدر تبدلها فأي وقت.
          </p>
        </div>
      </div>
              ...
              <div className="divide-y divide-gray-50">
                {NOTIF_TYPES.map((type) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <TIcon className={`h-4 w-4 ${isOn ? 'text-indigo-600' : 'text-slate-300'}`} />
                        <div>
                          <p className={`text-sm font-bold ${isOn ? 'text-slate-800' : 'text-slate-400'}`}>
                            {type.labelAr}
                          </p>
      ...
      <div className="flex justify-end">
        <button
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 font-bold text-sm disabled:opacity-50 shadow-sm"
        >
          {saving ? (
