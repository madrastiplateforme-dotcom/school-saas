...
function getDynamicSizes(totalRows: number) {
  if (totalRows <= 8) {
    return { contentPadding: 38, contentPaddingTop: 32, fontSize: 9, sectionMarginBottom: 10, rowPadding: 5, tableHeaderPadding: 5, infoGridPadding: 8, infoItemMarginBottom: 4, titleSize: 20, titleFrSize: 11, titleMarginTop: 8, titleMarginBottom: 12, headerMarginBottom: 10, signMarginTop: 15, signPaddingTop: 10, signLabelFrMarginBottom: 40 }
  }
  if (totalRows <= 14) {
    return { contentPadding: 30, contentPaddingTop: 26, fontSize: 8.5, sectionMarginBottom: 8, rowPadding: 4, tableHeaderPadding: 4, infoGridPadding: 6, infoItemMarginBottom: 3, titleSize: 18, titleFrSize: 10, titleMarginTop: 6, titleMarginBottom: 9, headerMarginBottom: 8, signMarginTop: 12, signPaddingTop: 8, signLabelFrMarginBottom: 32 }
  }
  if (totalRows <= 20) {
    return { contentPadding: 24, contentPaddingTop: 20, fontSize: 8, sectionMarginBottom: 6, rowPadding: 3, tableHeaderPadding: 3.5, infoGridPadding: 5, infoItemMarginBottom: 2, titleSize: 16, titleFrSize: 9, titleMarginTop: 4, titleMarginBottom: 7, headerMarginBottom: 6, signMarginTop: 10, signPaddingTop: 6, signLabelFrMarginBottom: 28 }
  }
  return { contentPadding: 20, contentPaddingTop: 16, fontSize: 7.5, sectionMarginBottom: 5, rowPadding: 2.5, tableHeaderPadding: 3, infoGridPadding: 4, infoItemMarginBottom: 2, titleSize: 14, titleFrSize: 8, titleMarginTop: 3, titleMarginBottom: 6, headerMarginBottom: 5, signMarginTop: 8, signPaddingTop: 5, signLabelFrMarginBottom: 22 }
}
...
export default function ContractPDF({ data }: { data: any }) {
  ...
  const totalRows = (data.items?.length || 0) + (data.installments?.length || 0)
  ...
  const styles = StyleSheet.create({
    ...
    kingdomAr: { fontSize: 10, fontWeight: 'bold', color: C.navy, textAlign: 'center', lineHeight: 1.5 },
    kingdomFr: { fontSize: 8, fontWeight: 'bold', color: C.navy, textAlign: 'center', marginTop: 0.5 },
    schoolAr: { fontSize: 13, fontWeight: 'bold', color: C.gold, textAlign: 'center', marginTop: 5, lineHeight: 1.4 },
    schoolMeta: { fontSize: 8, color: C.gray, textAlign: 'center', marginTop: 2, lineHeight: 1.5 },
    badge: {
      backgroundColor: C.navy, paddingHorizontal: 10, paddingVertical: 5,
      borderRadius: 4, minWidth: 90, textAlign: 'center',
    },
    badgeCancelled: { backgroundColor: ROSE },
    badgeText: { color: '#ffffff', fontSize: 8.5, fontWeight: 'bold', textAlign: 'center', lineHeight: 1.4 },
    ...
    sectionHeader: {
      ...
      paddingVertical: 5, paddingHorizontal: 10, marginBottom: 5,
    ...
    infoItem: {
      width: '50%', paddingVertical: 3, paddingHorizontal: 4,
      marginBottom: S.infoItemMarginBottom,
    },
    infoLabelAr: { fontSize: 7.5, color: C.gray, textAlign: 'right', marginBottom: 1, lineHeight: 1.5 },
    infoLabelFr: { fontSize: 6.5, color: C.gray, textAlign: 'right', fontStyle: 'italic' },
    infoValue: { fontSize: 9.5, color: C.dark, fontWeight: 'bold', marginTop: 1.5, textAlign: 'right', lineHeight: 1.5 },

    table: { borderWidth: 1, borderColor: C.navy, borderRadius: 3, overflow: 'hidden' },
    tableHeader: { flexDirection: 'row-reverse', backgroundColor: C.navy, paddingVertical: S.tableHeaderPadding },
    th: { color: '#ffffff', fontSize: 8, fontWeight: 'bold', textAlign: 'center', paddingHorizontal: 3, lineHeight: 1.4 },
    tableRow: {
      flexDirection: 'row-reverse',
      borderBottomWidth: 0.5, borderBottomColor: '#e5e5e5',
      borderBottomStyle: 'dotted', paddingVertical: S.rowPadding, alignItems: 'center',
    },
    tableRowAlt: { backgroundColor: '#fafafa' },
    tableRowLast: { flexDirection: 'row-reverse', paddingVertical: S.rowPadding, alignItems: 'center' },
    td: { fontSize: 8.5, color: C.dark, paddingHorizontal: 3, textAlign: 'center', lineHeight: 1.4 },
    tdRight: { fontSize: 8.5, color: C.dark, paddingHorizontal: 3, textAlign: 'right', lineHeight: 1.4 },
    ...
    statusBadge: { fontSize: 7.5, paddingHorizontal: 4, paddingVertical: 2, borderRadius: 4, textAlign: 'center' },
    statusPaid: { backgroundColor: '#D1FAE5', color: '#065F46' },
    statusPending: { backgroundColor: '#FEF3C7', color: '#92400E' },
    statusPartial: { backgroundColor: '#DBEAFE', color: '#1E40AF' },

    totalBox: { backgroundColor: C.lightGold, borderWidth: 1, borderColor: C.gold, borderRadius: 4, padding: 10, marginTop: 10 },
    totalRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', paddingVertical: 3, alignItems: 'center' },
    totalLabelAr: { fontSize: 9.5, color: C.navy, textAlign: 'right', fontWeight: 'bold', lineHeight: 1.5 },
    ...
    grandTotal: { fontSize: 12, fontWeight: 'bold', color: C.navy, lineHeight: 1.4 },

    notesBox: {
      backgroundColor: '#fdfdf9', borderWidth: 1, borderColor: C.line,
      borderStyle: 'dashed', borderRadius: 3, padding: 8, marginTop: 8,
    },
    notesLabel: { fontSize: 8, color: C.gray, marginBottom: 3, textAlign: 'right', fontWeight: 'bold', lineHeight: 1.5 },
    notesText: { fontSize: 9, color: C.dark, lineHeight: 1.7, textAlign: 'right' },

    footer: {
      flexDirection: 'row-reverse', justifyContent: 'space-between',
      marginTop: S.signMarginTop, paddingTop: S.signPaddingTop,
    ...
    signLine: { width: '100%', borderTopWidth: 1, borderTopColor: C.navy, marginTop: 15 },
    signText: { fontSize: 8, color: C.gray, textAlign: 'center', marginTop: 3, lineHeight: 1.4 },

    footerNote: { fontSize: 7, color: C.gray, textAlign: 'center', marginTop: 12, fontStyle: 'italic' },
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitleAr}>معلومات التلميذ</Text>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.table}>
              ...
              {data.items.map((item: any, i: number) => (
                <View key={i} style={[i === data.items.length - 1 ? styles.tableRowLast : styles.tableRow, i % 2 === 1 ? styles.tableRowAlt : {}]}>
                  <Text style={[styles.tdRight, styles.colService, { fontWeight: 'bold', color: C.navy }]}>{item.name}</Text>
                  <Text style={[styles.td, styles.colType]}>{item.type === 'monthly' ? 'شهري' : item.type === 'annual' ? 'سنوي' : 'مرة واحدة'}</Text>
                  <Text style={[styles.td, styles.colPrice]}>{item.price.toFixed(2)} DH</Text>
                  <Text style={[styles.td, styles.colDiscount]}>{item.discount > 0 ? `- ${item.discount.toFixed(2)} DH` : '-'}</Text>
                  <Text style={[styles.td, styles.colFinal, { fontWeight: 'bold', color: C.navy }]}>{item.finalPrice.toFixed(2)} DH</Text>
                </View>
              ))}
            </View>
          </View>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.table}>
                ...
                {data.installments.map((inst: any, i: number) => (
                  <View key={i} style={[i === data.installments.length - 1 ? styles.tableRowLast : styles.tableRow, i % 2 === 1 ? styles.tableRowAlt : {}]}>
                    <Text style={[styles.tdRight, styles.colService, { fontWeight: 'bold', color: C.navy }]}>{inst.description}</Text>
                    ...
                    <View style={styles.colStatus}>
                      <Text style={[styles.statusBadge, getStatusStyle(inst.status)]}>{getStatusLabel(inst.status)}</Text>
                    </View>
                  </View>
                ))}
              </View>
          ...
          <View style={styles.totalBox}>
            ...
            <View style={styles.totalRow}>
              ...
              <Text style={[styles.totalValue, { color: EMERALD }]}>{data.totalPaid.toFixed(2)} DH</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.totalLabelAr}>المتبقي</Text>
              <Text style={[styles.totalValue, { color: data.totalRemaining > 0 ? ROSE : EMERALD }]}>{data.totalRemaining.toFixed(2)} DH</Text>
            </View>
          </View>
